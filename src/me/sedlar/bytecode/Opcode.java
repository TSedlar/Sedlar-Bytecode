...
public enum Opcode {
	...
	public static Opcode[] ARITHMETIC_OPCODES = { IMUL, IDIV, IADD, ISUB, DMUL, DDIV, DADD, DSUB, FMUL, FDIV,
			FADD, FSUB, ISHL, ISHR, LSHL, LSHR, IXOR, LXOR, IAND, LAND };

	private static Opcode[] LOOKUP = new Opcode[256];
	private static Map<String, Opcode> NAME_LOOKUP = new HashMap<>();

	static {
		for (Opcode opcode : values()) {
			LOOKUP[opcode.id()] = opcode;
			NAME_LOOKUP.put(opcode.verbose, opcode);
		}
	}
	...
	/**
	 * Gets the matching opcode from the given name.
	 *
	 * @param opcode
	 *            The name of the opcode to match.
	 * @return the matching opcode for the given name.
	 */
	public static Opcode fromName(String opcode) {
		for (Map.Entry<String, Opcode> entry : NAME_LOOKUP.entrySet()) {
			if (entry.getKey().equalsIgnoreCase(opcode))
	...
	Opcode(int bytecode, String verbose, String suffix) {
		this.id = bytecode;
	...
	/**
	 * The information url of the opcode
	 *
	 * @return Information URL of the opcode
	 */
	public String url() {
		return JVM_SPEC_URL + suffix;
	}

	/**
	 * Check if the opcode is between the ordinals of the given opcodes.
	 *
	 * @param start
	 *            the range start
	 * @param end
	 *            the range end
	 * @return if the opcode is between the ordinals of the given opcodes
	 */
	public boolean withinRange(Opcode start, Opcode end) {
		return ordinal() >= start.ordinal() && ordinal() <= end.ordinal();
	}

	/**
	 * Get the default InstructionFilter for this opcode
	 *
	 * @return the default InstructionFilter for this opcode
	 */
	public InstructionFilter filter() {
		final Opcode self = this;
		return new InstructionFilter() {
			public boolean accept(AbstractInstruction ai) {
				return ai.opcode().equals(self);
			}
		};
	}
}
